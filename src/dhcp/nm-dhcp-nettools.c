...
static guint
fqdn_flags_to_wire (NMDhcpFqdnFlags flags)
{
	guint r = 0;

	/* RFC 4702 section 2.1 */
	if (flags & NM_DHCP_FQDN_FLAG_SERVER_UPDATE)
		r |= (1 << 0);
	if (flags & NM_DHCP_FQDN_FLAG_ENCODED)
		r |= (1 << 2);
	if (flags & NM_DHCP_FQDN_FLAG_NO_UPDATE)
		r |= (1 << 3);
           ...
           GError **error)
{
	...
	if (hostname) {
		if (nm_dhcp_client_get_use_fqdn (client)) {
			...
			flags = nm_dhcp_client_get_fqdn_flags (client);
