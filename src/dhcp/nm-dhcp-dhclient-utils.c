                ...
                NMDhcpFqdnFlags fqdn_flags)
{
	...
	if (hostname) {
		if (use_fqdn) {
			...
			if (fqdn_flags & NM_DHCP_FQDN_FLAG_ENCODED)
				g_string_append (str, FQDN_TAG_PREFIX "encoded on;\n");
			if (fqdn_flags & NM_DHCP_FQDN_FLAG_SERVER_UPDATE)
				g_string_append (str, FQDN_TAG_PREFIX "server-update on;\n");
			if (fqdn_flags & NM_DHCP_FQDN_FLAG_NO_UPDATE)
                                ...
                                GBytes **out_new_client_id)
{
	...
	if (addr_family == AF_INET) {
		add_ip4_config (new_contents, client_id, hostname, use_fqdn, fqdn_flags);
		...
		add_hostname6 (new_contents, hostname, fqdn_flags);
